...
function openEdit(oblast) {
    editingOblast.value = oblast
    form.name = oblast.name
    form.is_active = oblast.is_active
    form.clearErrors()
    showModal.value = true
}
...
function submit() {
    if (editingOblast.value) {
        form.put(route('oblasts.update', editingOblast.value.id), {
            onSuccess: closeModal,
        })
    } else {
        form.post(route('oblasts.store'), {
            onSuccess: closeModal,
        })
    }
}

function destroy(oblast) {
    if (!confirm(t('oblasts.delete_confirm'))) { return }
    router.delete(route('oblasts.destroy', oblast.id))
}

const currentPage = computed(() => props.oblasts?.current_page ?? 1)
const lastPage = computed(() => props.oblasts?.last_page ?? 1)
const oblastList = computed(() => props.oblasts?.data ?? [])
</script>
        ...
        <div class="space-y-4">
            ...
            <div class="overflow-hidden rounded-xl bg-white shadow-sm dark:bg-slate-800">
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                        <thead class="bg-gray-50 dark:bg-slate-700/50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-slate-400">{{ t('oblasts.name') }}</th>
                                <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-slate-400">{{ t('oblasts.status') }}</th>
                                <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-slate-400">{{ t('oblasts.created_at') }}</th>
                                <th class="px-6 py-3 text-right text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-slate-400">{{ t('oblasts.actions') }}</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100 dark:divide-slate-700">
                            <tr v-if="oblastList.length === 0">
                                <td colspan="5" class="px-6 py-12 text-center text-sm text-gray-400 dark:text-slate-500">
                                    {{ t('oblasts.empty') }}
                                </td>
                            </tr>
                            <tr
                                v-for="(oblast, index) in oblastList"
                                :key="oblast.id"
                            ...
                            >
                                ...
                                <td class="px-6 py-4 text-center">
                                    ...
                                    >
                                        <span
                                            :class="oblast.is_active ? 'bg-green-500 dark:bg-green-400' : 'bg-red-400'"
                ...
                <div v-if="lastPage > 1" class="flex items-center justify-between border-t border-gray-100 px-6 py-4 dark:border-slate-700">
                    <p class="text-sm text-gray-500 dark:text-slate-400">{{ t('oblasts.title') }}</p>
                    <div class="flex gap-1">
                        <Link
                            v-for="page in lastPage"
                            :key="page"
                            :href="route('oblasts.index', { page })"
